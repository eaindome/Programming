/*
Write a program to print the area of a rectangle by creating 
a class named 'Area' taking the values of its length and 
breadth as parameters of its constructor and having a 
function named 'returnArea' which returns the area of the 
rectangle. Length and breadth of the rectangle are entered 
through keyboard.
*/

#include <iostream>

using namespace std;

class Average {
    private:
        float num1, num2, num3;

    public:
        Average(float n1, float n2, float n3):num1(n1),num2(n2),num3(n3){}

        float average(){
            return (num1+num2+num3)/3;
        }
};


int main() {
    float num1, num2, num3;

    cout<<"Enter 3 numbers separated with spaces: ";
    cin>>num1>>num2>>num3;

    Average avg(num1, num2, num3);

    cout<<"The average of the numbers entered = "<<avg.average()<<"."<<endl;

    return 0;
}
