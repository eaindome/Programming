<script lang="ts">
  ...
  let inventoryAlerts = [
    {
      id: 'ALT-1001',
      type: 'critical',
      message: 'Amoxicillin 500mg - Below critical threshold (20 units remaining)',
      timestamp: '2025-03-11T08:30:00'
    },
    {
      id: 'ALT-1002',
      type: 'warning',
      message: 'Lisinopril 10mg - Expiring in 14 days',
      timestamp: '2025-03-11T09:15:00'
    },
    {
      id: 'ALT-1003',
      type: 'info',
      message: 'Batch TRX-5430 requires verification',
      timestamp: '2025-03-10T16:45:00'
    },
    {
      id: 'ALT-1004',
      type: 'critical',
      message: 'Metformin 850mg - Stock depleted',
      timestamp: '2025-03-10T11:20:00'
    },
    {
      id: 'ALT-1005',
      type: 'warning',
      message: 'Atorvastatin 20mg - Below reorder level (45 units remaining)',
      timestamp: '2025-03-09T14:10:00'
    }
  ];
  ...
  function getAlertBadgeType(type: string): 'success' | 'warning' | 'error' | 'info' | 'default' {
    switch (type) {
      case 'critical':
        return 'error';
      case 'warning':
...
<div class="p-6 bg-[#F9FAFB] min-h-screen">
  <div class="mb-6">
    <h1 class="text-2xl font-bold text-gray-800">Inventory Management Dashboard</h1>
    <p class="text-gray-500 mt-1">Overview of inventory, transactions, and alerts</p>
  </div>
  
  <!-- Quick Action Buttons -->
  <div class="flex flex-wrap gap-4 mb-6">
    <Button variant="primary" icon="alert-circle">View Alerts</Button>
    <Button variant="secondary" icon="refresh">Update Inventory</Button>
    <Button variant="outline" icon="file-text">Generate Report</Button>
  </div>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-6">
    ...
    <Stat 
      ...
      value={inventorySummary.criticalAlerts} 
      change={3} 
      icon="alert-octagon" 
      color="red" 
    />
    ...
    <Chart 
      ...
      options={{ 
        plugins: { 
          ...
          title: { display: true, text: 'Last 6 Months' }
        },
        scales: {
          y: { beginAtZero: false }
        }
  ...
  <!-- Alerts Section -->
  <Card title="Inventory Alerts" icon="alert-circle">
          ...
          <div class="p-4 border rounded-lg flex items-start">
            ...
            <div class="flex-1">
              <div class="flex justify-between items-start">
                <div>
                  <Badge text={alert.type.charAt(0).toUpperCase() + alert.type.slice(1)} type={getAlertBadgeType(alert.type)} />
                  <p class="mt-1 font-medium">{alert.message}</p>
                </div>
                <span class="text-sm text-gray-500">{formatDateTime(alert.timestamp)}</span>
              </div>
              <div class="mt-3">
