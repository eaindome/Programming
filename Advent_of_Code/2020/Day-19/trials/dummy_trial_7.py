...
def count_valid_messages(rules, messages):
    def exclusively_matches_rule_0(message):
        valid = is_valid_message(rules, message)
        if valid:
            for rule_num in rules:
                if rule_num != 0:
                    if is_valid_message(rules, message) is not None:
                        return False
        return valid

    count = sum(1 for message in messages if exclusively_matches_rule_0(message))
    return count
...
# Count how many messages exclusively match rule 0
valid_count = count_valid_messages(rules, messages)
print(f"Number of valid messages exclusively matching rule 0: {valid_count}")
