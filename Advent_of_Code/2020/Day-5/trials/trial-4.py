def find_highest_seat_ID(binary_spaces):
    ...
    print(f"Binary spaces b4: {binary_spaces}")
    ...
    sort_seats = sorted(seat_IDs)

    # iterate through orted list
    for i in range(len(sort_seats)-1):
        # if the difference between the next seat and the current seat is greater than 1
        if sort_seats[i+1] - sort_seats[i] > 1:
            # found a gap in the sequence which indicates a missing seat
            missing_seat_ID = sort_seats[i] + 1
            return highest_seat_ID, missing_seat_ID
...
def main():
    ...
    highest_seat_ID, missing_seat_ID = find_highest_seat_ID(binary_spaces)
    print(f"The highest seat ID is: {highest_seat_ID}\n"
          f"The ID of your seat is: {missing_seat_ID}")
